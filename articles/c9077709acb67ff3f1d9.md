...
Kindle で書籍を購入した際に Amazon からメールが届きます。そのメールから使いたい Asin コードを抜き出して、購入した書籍をブクログに登録します。その際、処理のログをスプレッドシートへ登録します。

これらのスクリプトは、TypeScript で書いていて GitHub でバージョン管理しています。デプロイは GitHub Actions を使って Google Apps Script へ自動で行い、Google Apps Script のトリガーで定期実行させます。
...
ローカルに `appsscript.json` が作成されていれば OK です。

## GAS で Gmail を検索する

実際に `./src` ディレクトリを作成して TS ファイルを作成して書いていくわでけですが Google サービスを扱う場合は GAS を使うと下記のように非常に簡単です。
...
`getSheetByName()` メソッドの引数にはシートの名前をしていました。事前に好きな名前にしておきましょう。
