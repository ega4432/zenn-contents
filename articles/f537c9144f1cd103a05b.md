---
title: "Tekton Pipeline でプライベートリポジトリをクローンする"
emoji: "😊"
...
OpenShift 上でのアプリケーション開発では CI/CD に Tekton を使用する機会が多いです。（tekton についての説明を入れる。）そこで プライベートリポジトリでソースコードを管理している場合、Tekton での CI/CD の実装する際に一手間必要だったので、備忘録として残しておきます。
...
1 つ目は、クローンしたソースコードを Pipeline 内で使い回すために PVC を定義しています。一般的に CI/CD を実装する際は、ビルドやテストなど複数 Task を跨ぐことが多く、そのように Task 間でデータの受け渡しが必要となるためこのように PVC, PV を使用しています。

2 つ目は、Pipeline で定義した `ssh-creds` ワークスペースに Secret をバインドさせている点です。こうすることで、`git-clone` Task で SSH 認証を可能にしています。
